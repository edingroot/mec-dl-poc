...
class CameraFragment : Fragment(), View.OnClickListener,
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        file = File(activity!!.getExternalFilesDir(null), Constants.PIC_FILE_NAME)
    }
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                val largest = Collections.max(
                    listOf(*map.getOutputSizes(ImageFormat.JPEG)), // Arrays.asList(*map.getOutputSizes(ImageFormat.JPEG)),
                    CompareSizesByArea()
                )
    ...
    private fun openCamera(width: Int, height: Int) {
        val permission =
            ContextCompat.checkSelfPermission(activity as Context, Manifest.permission.CAMERA)
